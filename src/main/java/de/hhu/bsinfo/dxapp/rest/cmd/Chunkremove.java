...
public class Chunkremove extends AbstractRestCommand {
    ...
    public void register(Service server, ServiceHelper services) {
        server.get("/chunkcreate", (request, response) -> {
            ...
            if (stringCid == null) {
                return createError("Invalid Parameter, please use: /chunkremove?cid=[CID]");
            }

            long cid = ChunkID.parse(stringCid);

            if (cid == ChunkID.INVALID_ID) {
                return createError("ChunkID invalid");
            }else if (ChunkID.getLocalID(cid) == 0){
                return createError("Removal of index chunk is not allowed");
            }

            if (services.getService(ChunkService.class).remove().remove(cid) != 1) {
                return createError("Removing chunk with ID "+ChunkID.toHexString(cid)+" failed");
            } else {
